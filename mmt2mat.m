function [MMT,MMT_Site_Info,MMT_Transects,MMT_Field_Config,...
        ...
        %
        % This code was originally developed and used in 2010 as a Matlab
        % function. In 2013 DSM has integrated that function into the
        % current class.
        %
        % Developers:
        % David S. Mueller, Hydrologist, Office of Surface Water
        % Nathan Reynolds, Student, Illinois Water Science Center
        % Jeramiah Lant, Hydrologist, Kentucky Water Science Center
        % Frank Engel, Hydrologist, Illinios Water Science Center
        %
        % Revisions:
        % From QRev 1.93
        ...
        % ========================================================================
            ...
            if isstruct(s.WinRiver.Project.Locked)
                MMT.Locked = s.WinRiver.Project.Locked;
            else
                MMT.Locked = nan;
            end
            ...
            % all data are stored as text except for 'Water_Temperature' 
            s_end = 'Text';
            for i = 1:length(fields_siteinfo)   
                if ~isempty(s.WinRiver.Project.Site_Information.(fields_siteinfo{i}).(s_end))
                    if strcmp(fields_siteinfo{i},'Water_Temperature')
                        ...
                        if MMT_Site_Info.(fields_siteinfo{i}) < -100
                            MMT_Site_Info.(fields_siteinfo{i})=[];
                        end
                    else
                        MMT_Site_Info.(fields_siteinfo{i}) =...
            ...
            % If there is only 1 transect, convert it to a cell.
            ...
            for i = 1:length(trans)
                ...
                if isfield(trans{i},'Note')
                    ...
                    % If there is only 1 note, convert it to a cell.
                    [note] = convert2cell(note);
                    for n = 1:length(note)
                        MMT_Transects.NoteFileNo{i,n}=MMT_Transects.Number{i,1};
                ...
                % If there is only 1 configuration, convert it to a cell.
...
%                                 MMT_MB_Transects.Path{i}=files{1}.Attributes.PathName;
%                                 MMT_MB_Transects.Files{i}=files{1}.Text;
%                                 MMT_MB_Transects.Number{i} =files{1}.Attributes.TransectNmb;
                                
                                if isfield(QAQC,'Moving_Bed_Test.Moving_Bed_Test_Summary.MB_Tests')
                                    % Type of moving-bed test
                                    type=QAQC.Moving_Bed_Test.Moving_Bed_Test_Summary.MB_Tests.(['Index_',num2str(i-1)]).Type.Text;
                                    if strcmp(type,'1')
                                        MMT_MB_Transects.Type(i)={'Loop'};
                                    elseif strcmp(type,'2')
                                        MMT_MB_Transects.Type(i)={'Stationary'};
                                    end
                                else
                                    fileName=MMT_MB_Transects.Files{i,1};
                                    idx=find(fileName=='.',1,'last');
                                    if strcmp(fileName(idx-3:idx-1),'SBT')
                                        MMT_MB_Transects.Type(i)={'Stationary'};
                                    elseif strcmp(fileName(idx-3:idx-1),'LBT')
                                        MMT_MB_Transects.Type(i)={'Loop'};
                                    else
                                        question=['What type of moving-bed test is ' fileName '?'];
                                        MMT_MB_Transects.Type(i)={questdlg(question,'Moving-Bed Test','Loop','Stationary','Loop')};
                                    end % if type
                                end % if isfield
        ...
        % of configuration variables in the structure to be constructed through multiple calls.
            ...
            for i=1:length(fields_commands)
                ...
                if strcmp(f,'Text')
                    MMT_Config.(fields_commands{i}){iTrans,1} = commands.(fields_commands{i}).Text;
                else
                    for k = 1:length(f)
            ...
            % ---
            % Assign local variable
            names=fieldnames(mmtDataChild);
            
            if ismember('GPS',names)
            ...
            if ismember('Root_Directory',names)
